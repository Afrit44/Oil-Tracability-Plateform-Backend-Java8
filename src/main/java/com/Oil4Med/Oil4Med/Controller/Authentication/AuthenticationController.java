...
public class AuthenticationController {
    ...
    public ResponseEntity<AuthenticationResponse> authenticate(AuthenticationRequest request){
        ...
        } else if (consumerRepository.findByEmail(request.getEmail())!=null) {
            final String jwt = jwtUtil.generateToken(userDetails) ;
            return ResponseEntity.ok(AuthenticationResponse.builder().jwtToken(jwt).role("Conusmer").build());
        } else if (farmerRepository.findByEmail(request.getEmail())!=null) {
            final String jwt = jwtUtil.generateToken(userDetails) ;
            return ResponseEntity.ok(AuthenticationResponse.builder().jwtToken(jwt).role("Farmer").build());
        } else if (millManagerRepository.findByEmail(request.getEmail())!=null) {
            final String jwt = jwtUtil.generateToken(userDetails) ;
